...
RUN export DEBIAN_FRONTEND=noninteractive && apt-get update && apt-get install -y software-properties-common && \
    ...
    apache2 libapache2-mod-php7.2 php7.2 php7.2-xml php7.2-gd php7.2-opcache php7.2-mbstring php7.2-zip \
    php7.2-mysql php7.2-tidy php7.2-curl php7.2-sqlite3 php7.2-xdebug php7.2-intl \
    gnupg2 && curl -sL https://deb.nodesource.com/setup_12.x | bash -  && \
    apt-get install -y nodejs && \
    curl -sS https://getcomposer.org/installer | php && mv composer.phar /usr/local/bin/composer && \
    a2enmod rewrite expires && \
    apt-get clean && apt-get autoclean && a2enmod proxy proxy_http proxy_wstunnel
